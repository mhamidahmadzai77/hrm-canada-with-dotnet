$(document).ready(function () {  
    ...
    $('#add-new-appeal').click(function (e) {  
        ...
        if (formIsValid) {  
            ...
            // AJAX call  
            $.ajax({  
                url: '/Appeals/Add', // Update with your controller/action URL  
                type: 'POST',  
                data: formData,  
                contentType: false,  
                processData: false,  
                success: function (response) {  
                    if (response.redirectTo) {  
                        ...
                        swal("Error", "You have entered a duplicate title, please write a different title.", {  
                            icon: "error", buttons: " Ok "  
                        });  
                    } else {  
...
function validateImages() {  
    ...
    if (file) {  
        document.getElementById('image2-error').style.display = "none";  
    } else {  
        document.getElementById('image2-error').style.display = "block";  
    }  
    ...
    if (file) {  
        document.getElementById('image3-error').style.display = "none";  
    } else {  
    ...
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                $.ajax({
                    url: '/Appeals/DeleteAppeal', // Update with your controller/action URL  
                    type: 'POST',
                    data: { id: id },
                    success: function (response) {
                        if (response === "deleted") {
